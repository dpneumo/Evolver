...
require_relative '../../../../utilities/critters/rabbit/fertility'

class Tests::Utilities::Critters::Rabbit::FertilityTest < Minitest::Test
  def setup
    resource = MockToolbox.new(resource: MockResource9).resource
    @fertility = Utilities::Critters::Rabbit::Fertility.new(resource: resource)
  end
  ...
  def test_rabbit_probability_converts_a_negative_result_to_0
    age = 0
    color_id = 4
    assert_includes 0..1, @fertility.probability(age: age, color_id: color_id)
  end
  ...
  def test_rabbit_probability_ignores_a_color_id_lt_0
    age = 4
    color_id = -2
    assert_equal 0, @fertility.probability(age: age, color_id: color_id)
  end

  def test_rabbit_probability_ignores_a_non_integer_color_id
    age = 4
    color_id = 2.4
    assert_equal 0, @fertility.probability(age: age, color_id: color_id)
  end
  ...
  def test_rabbit_probability_returns_0_if_color_id_is_nil
    age = 4
    color_id = nil
    assert_equal 0, @fertility.probability(age: age, color_id: color_id)
  end
