...
class MortalityTest < Minitest::Test
  ...
  def test_probability_returns_a_result_between_0_and_1
    age = 4
    color_id = 2
    assert_includes 0..1, @mortality.probability(age: age, color_id: color_id)
  end
  ...
  def test_probability_converts_a_result_gt_1_to_1
    age = 4
    color_id = 0
    assert_equal 1.00, @mortality.probability(age: age, color_id: color_id)
  end

  def test_probability_ignores_an_age_lt_0
    age = -1
    color_id = 2
    assert_equal 0, @mortality.probability(age: age, color_id: color_id)
  end
